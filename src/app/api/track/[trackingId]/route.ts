...
export async function GET(
  ...
  try {
    ...
    const flight = await getFlightByTrackingId(id);
    if (!flight)
    ...
    return NextResponse.json({
      flight: {
        ...
        origin: {
          code: plan.legs[0].from.code,
          city: plan.legs[0].from.city,
          country: plan.legs[0].from.country,
          lat: plan.legs[0].from.lat,
          lng: plan.legs[0].from.lng,
        },
        destination: {
        ...
        consignee: {
          name: flight.consignee.name,
          company: flight.consignee.company,
        },
